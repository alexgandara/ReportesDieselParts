...
public class printPDFA4Item {
	...
	public static void imp_factura(String _file_xml, factura_cabecera Cabecera, factura_detalle[] Detalle, int _lineas_de_la_factura, String _file_pdf, String _file_jpg) throws DocumentException, IOException {
		...
		if (Cabecera.get_direccion_receptor().length()>60) {
			int _tam = Cabecera.get_direccion_receptor().length();
			String _dir01 =  Cabecera.get_direccion_receptor().substring(0, 60);
			String _dir02 =  Cabecera.get_direccion_receptor().substring(60, _tam);
		...
		String _salida = "-";
		if (Cabecera.get_orden_compra().equals("-")) {
			_salida = "-";
		} else {
			_salida = "("+Cabecera.get_orden_compra()+")N°:"+Cabecera.get_numero_orden_compra();
		}
		...
		canvas.saveState();                               // q
		canvas.beginText();                               // BT
		canvas.moveText(100, 621);                         // 36 788 Td
		canvas.setFontAndSize(bf, 9); // /F1 12 Tf
		canvas.showText(_salida);	        // (Hello World)Tj
		canvas.endText();                                 // ET
		canvas.restoreState();                            // Q
		...
		canvas.saveState();                               // q
		canvas.beginText();                               // BT
		canvas.moveText(490, 664);                         // 36 788 Td
		canvas.setFontAndSize(bf, 9); // /F1 12 Tf
		...
		canvas.saveState();                               // q
		canvas.beginText();                               // BT
		canvas.moveText(490, 653);                         // 36 788 Td
		canvas.setFontAndSize(bf, 9); // /F1 12 Tf
		...
		String _fecha_final = _fecha.substring(8,10)+"/"+_fecha.substring(5,7)+"/"+_fecha.substring(0,4);


		//fecha vencimiento
		canvas.saveState();                               // q
		canvas.beginText();                               // BT
		canvas.moveText(490, 642);                         // 36 788 Td
		canvas.setFontAndSize(bf, 9); // /F1 12 Tf
		...
		if (Cabecera.get_fecha().equals(_fecha_final)) {
			...
			canvas.saveState();                               // q
			canvas.beginText();                               // BT
			canvas.moveText(490, 621);                         // 36 788 Td
			canvas.setFontAndSize(bf, 9); // /F1 12 Tf
			canvas.showText("CONTADO");
			canvas.endText();                                 // ET
			canvas.restoreState();                            // Q
			...
			canvas.saveState();                               // q
			canvas.beginText();                               // BT
			canvas.moveText(490, 623);                         // 36 788 Td
			canvas.setFontAndSize(bf, 9); // /F1 12 Tf
			canvas.showText("CREDITO");
			canvas.endText();                                 // ET
			canvas.restoreState();                            // Q
		...
		canvas.saveState();                               // q
		canvas.beginText();                               // BT
		canvas.moveText(490, 609);                         // 36 788 Td
		canvas.setFontAndSize(bf, 9); // /F1 12 Tf
		canvas.showText(Cabecera.get_cotizacion());	        // (Hello World)Tj
		canvas.endText();                                 // ET
		canvas.restoreState();                            // Q
		...
		canvas.saveState();                               // q
		canvas.beginText();                               // BT
		canvas.moveText(495, 610);                         // 36 788 Td
		canvas.setFontAndSize(bf, 9); // /F1 12 Tf
		...
		canvas.setFontAndSize(bf, 9); // /F1 12 Tf
		canvas.showText(Formato.dinero(Cabecera.get_total_descuentos()));
		canvas.endText();                                 // ET
		canvas.restoreState();                            // Q
		...
		canvas.setFontAndSize(bf, 9); // /F1 12 Tf
		canvas.showText(Formato.dinero(Cabecera.get_total_anticipo()));
		canvas.endText();                                 // ET
		canvas.restoreState();                            // Q  
		...
		canvas.saveState();                               // q
		canvas.beginText();                               // BT
		canvas.moveText(30, 274);                         // 36 788 Td
		canvas.setFontAndSize(bf, 8); // /F1 12 Tf
		canvas.showText("Observaciones : "+Cabecera.get_observaciones());
		// (Hello World)Tj
		...
		canvas.saveState();                               // q
		canvas.beginText();                               // BT
		canvas.moveText(37, 240);                         // 36 788 Td
		canvas.setFontAndSize(bf, 8); // /F1 12 Tf
		if (Cabecera.get_moneda().equals("PEN")) {
		...
		for (int i=0; i<_lineas_de_la_factura; i++) {
			...
			if (Detalle[i].get_descripcion()!=null) { 	 
				if (Detalle[i].get_descripcion().trim().length()>0) {
					...
					Chunk _descripcion = new Chunk(Formato.padRight(Formato.cadena63(Detalle[i].get_descripcion()),72));
				...
				//	Chunk _precio = new Chunk(Formato.dinero((Detalle[i].get_subtotal()+Detalle[i].get_igv()/Detalle[i].get_cantidad())));
					Chunk _precio_con_igv = new Chunk(Formato.dinero(Detalle[i].get_precio_unitario()*1.18));
					Chunk _precio = new Chunk(Formato.dinero( Detalle[i].get_precio_unitario()*1.18));
					Chunk _importe = new Chunk(Formato.dinero(Detalle[i].get_subtotal()+Detalle[i].get_igv()));
				...
				//  	_lineas_de_la_factura
					...
					if (Detalle[i].get_codigo()!=".") {
						if (Detalle[i].get_codigo().equals(".")) {
							...
							_item++;
							Chunk _item_Chunk = new Chunk(Formato.item(_item));
							_item_Chunk.setFont(console);
							_linea01.add(_item_Chunk);
							_linea01.add(_espacio);
							_linea01.add(_espacio);
